 ...
 * - Subtle pulse animation to draw attention (respects prefers-reduced-motion)
 * - Tooltip shown once on first visit, then remembered in sessionStorage
 * - Respects mobile safe area insets
 * - Uses IntersectionObserver instead of scroll listener
 */
...
export function WhatsAppFAB() {
  ...
  // Use IntersectionObserver to detect scroll past threshold
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        // When sentinel leaves viewport (scrolled past), show FAB
  ...
  return (
    <>
      ...
      >
        ...
        >
          {/* Pulse ring — respects prefers-reduced-motion via CSS */}
          <span
            className="absolute inset-0 rounded-full bg-[#25D366]/30 animate-ping motion-reduce:animate-none"
